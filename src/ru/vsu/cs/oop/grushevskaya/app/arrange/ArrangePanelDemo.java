...
public class ArrangePanelDemo extends JPanel implements MouseListener, MouseMotionListener {
    ...
    public ArrangePanelDemo(GayFrame frame, String playerName) {
        ...
        {
            try {
                frame.getBattleField(new BattleField(ships));
            } catch (Exception ex) {
                ex.printStackTrace();
            }
        });
        button.setBounds(50, 490, 100, 30);
        add(button);

        JLabel label = new JLabel(String.format("%s, расставьте корабли", playerName));
        label.setBounds(50, 460, 500, 30);
        add(label);
    ...
    private void createShip(GraphicsCell[] graphicsCells) {
        ...
        if ((col1 == col2 || row1 == row2) && Math.abs(col2 - col1) < 4 && Math.abs(row2 - row1) < 4) {
            int widthOfTheShip = Math.abs(col1 - col2);
            int heightOfTheShip = Math.abs(row1 - row2);

            int checkForAvailable = Math.max(widthOfTheShip, heightOfTheShip) + 1;
            int counterAvailable = 0;
            for (Ship ship : ships) {
                if (ship.getDecks().size() == checkForAvailable) {
                    counterAvailable++;
                }
            }

            if (5 - counterAvailable - checkForAvailable != 0) {
...
//        if (graphicsCells[0].getScreenRow() < graphicsCells[1].getScreenRow()) {
//            GraphicsCell temp = graphicsCells[0];
//            graphicsCells[0] = graphicsCells[1];
//            graphicsCells[1] = temp; // todo: не работает емае((((
//        }
